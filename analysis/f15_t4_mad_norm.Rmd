...
I selected **32** transcripts with depth between 30 ~ 300[^1]. 
...
#load file
prediction_dir = "~/PhD_proj/pipeline/Validation/correct_prodiction_f15_t4_mad_norm/"
...
print(paste0("log likelihood (normal distribution) accuracy: ", mean(prediction$accuracy_loglik_score)))
...
```{r}
data <- melt(prediction[c("accuracy_loglik_score", "accuracy_manhattan", "accuracy_z_score","accuracy_mapper_support")])
...
In this section, the performance is evaluated over each squiggle assignment. Since the overall accuracy is better when using **log likelihood** as the distance metrics, I show here its results only.
```{r Load File}
library(ggplot2)
library(reshape2)
